...
class YanZHeng{
    ...
    public function pic($id = '')
    {
        ...
        if ($this->useZh) {
            ...
            for ($i = 0; $i < $this->length; $i++) {
                $code[$i] = iconv_substr($_SESSION['chars'], floor(mt_rand(0, mb_strlen($_SESSION['chars'], 'utf-8') - 1)), 1, 'utf-8');
	...
	$code2   = []; 
          ...
          for ($i = 0; $i < 16; $i++) {
                $code2[$i] = iconv_substr($this->zhSet, floor(mt_rand(0, mb_strlen($this->zhSet, 'utf-8') - 1)), 1, 'utf-8');
           }
           session_start();
           $_SESSION['chars'] = implode( $code2 );
          return $_SESSION['chars'];
    } 
    
    
    private function _writeNoise()
    {
        $codeSet = '补咱芽永瓦倾阵碳演威附牙芽永瓦斜灌欧献顺猪洋腐请透司危括脉宜笑若尾束壮暴企菜穗楚汉愈绿拖牛份染既秋遍锻玉夏疗尖殖井费州访吹荣铜沿替滚';
         $ttfPath = __DIR__ . '/';
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {      
               imagettftext($this->_image, 15, mt_rand(-40, 40), mt_rand(-10, $this->imageW), mt_rand(-10, $this->imageH), $noiseColor, $this->fontttf, iconv_substr($codeSet, floor(mt_rand(0, mb_strlen($codeSet, 'utf-8') - 1)), 1, 'utf-8'));       
            }
        }
    }
